...
class Logs():
    ...
    def __init__(self, ficheiro, localhost_ip, timeout, tempo_inicial, tempo_config, deporto):
        self.lock = Lock()
        ...
        try:
            f = open(ficheiro, "x")
            if self.deporto == True:
                put = self.estrutra(tempo_inicial, "ST", localhost_ip, f"{timeout} deporto")
            else:
