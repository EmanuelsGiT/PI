...
class ServidorPrincipal():
    ...
    def __init__(self, time_inicial):
        ...
        self.deporto = True
        self.logs = Logs(self.config["LG"], self.host_ip, 0, time_inicial, time_read_config, self.deporto)  # falta adicionar timeout para servidor
        time_bd = time_now()
        self.db = parseDB(self.db_path)
        self.logs.ev(time_bd, self.db_path)

    def __init__(self, time_inicial, config_path, bd_path, deporto):
        time_read_config = time_now()
        self.db_path = bd_path
        self.config = parseConfig(config_path)
        self.host_ip = str(ni.ifaddresses('eth0')[ni.AF_INET][0]['addr'])
        self.deporto = deporto
        self.logs = Logs(self.config["LG"], self.host_ip, 0, time_inicial, time_read_config, self.deporto)  # falta adicionar timeout para servidor
        time_bd = time_now()
