...
def exec_SP(config, bd, deporto, root):
    time_inicial = time_now()
    sp = ServidorPrincipal(time_inicial, config, bd, deporto)
    if not root:
        Thread(target=sp.tcp_connection, args=()).start()
    udp_connection(sp)

def exec_SS(config, deporto):
    time_inicial = time_now()
    ss = ServidorSecundario(time_inicial, config, deporto)
    while True:
        try:
            ss.tzSS()
            break
        ...
        try:
            ...
            if sys.argv.count('-s') == 1:
                ...
                if sys.argv.count('-root') == 1: # e importante saber quando e um servidor root para pois nao existe transferencia de zona
                    exec_SP(config,bd,deporto,True)
                exec_SP(config,bd,deporto,False)
            else:
