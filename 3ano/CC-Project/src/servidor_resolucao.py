...
import sys # vai receber como argumento a lista de servidores de topo e uma flag sobre o modo deporto
import time
...
# python3 servidor_resolucao.py ./ListaST/st.txt -deporto

# classe do servidor de resolução
class ServidorResolucao:

    def __init__(self, modoDeporto):
        time_inicial = time_now()
        self.deporto = modoDeporto
        self.SDT = parseSDT(sys.argv[1])
        self.host_ip = str(ni.ifaddresses('eth0')[ni.AF_INET][0]['addr'])
        self.logs = Logs("./log/all-log.txt", self.host_ip, 0, time_inicial, None, self.deporto)
...
def main():
    if(len(sys.argv) > 2):
        ...
        modoDeporto = False
    sr = ServidorResolucao(modoDeporto)
    sr.Clicomm()
