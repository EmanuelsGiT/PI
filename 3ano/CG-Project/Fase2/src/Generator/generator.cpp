...
int main(int argc, char* argv[])
{
    ...
    else
    {
        ...
        {
            float radius_in = atof(argv[2]);
            float radius_out = atof(argv[3]);
            int slices = atoi(argv[4]);
            int stacks = atoi(argv[5]);
            fig = calcTorus(radius_in, radius_out, slices, stacks);
            if (write3D(fig,argv[6])) 
                printf("Torus writen successfuly!\n");
        ...
        else if (strcmp(argv[1],"help") == 0)
        {
            ...
            printf("- generator cone 1(radius) 2(height) 4(slices) 3(stacks) cone.3d(file.3d)\n");
            printf("- generator plane 1(length) 3(divisions) plane.3d(file.3d)\n");
            printf("- generator torus 2(radius_in) 2(radius_out) 4(slices) 3(stacks) torus.3d(file.3d)\n");
        }
    }
    return 0;
}