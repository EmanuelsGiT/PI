...
figure generate::calcTorus(float radius_in, float radius_out, int slices, int stacks)
{
    ...
    for (int i = 0; i < slices; i++) {
        for (int j = 0; j < stacks; j++) {
            ...
            torus.addPoint((radius_in + radius_out * cos(phi + delta2)) * cos(theta + delta1), (radius_in + radius_out * cos(phi + delta2)) * sin(theta + delta1),
                radius_out * sin(phi + delta2));

            torus.addPoint((radius_in + radius_out * cos(phi + delta2)) * cos(theta + delta1), (radius_in + radius_out * cos(phi + delta2)) * sin(theta + delta1),
                radius_out * sin(phi + delta2));

            torus.addPoint((radius_in + radius_out * cos(phi + delta2)) * cos(theta), (radius_in + radius_out * cos(phi + delta2)) * sin(theta),
                radius_out * sin(phi + delta2));
